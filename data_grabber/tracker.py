...
def get_performance(summoner_name: str) -> plt.figure:
    ...
    for i in range(len(last_matches)):
        ...
        champion_id, lane, kills, deaths, assists, gold_earned, total_minions_killed, neutral_minions_killed, \
            total_damage_dealt_to_champions, vision_score, \
            win = lol_api.parse_useful_participant_match_info(participant_match_info)
        ...
        df.loc[len(last_matches) - i - 1] = [lane, champion_name, win, kda, gold_per_min,
                                             dmg_per_min, vis_per_min, minions_per_min]
